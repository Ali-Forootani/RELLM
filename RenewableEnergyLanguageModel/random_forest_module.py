...
def train_and_predict_ghgAbateFMs_ensemble(
    df_ghgAbateFMs: pd.DataFrame,
    final_feature_array_all,
    n_folds: int = 5
):
    # Step 0: Ensure final_feature_array_all is a DataFrame
    ...
    # Step 2: Merge with costTechFMs target
    df_costTechFMs_renamed = df_ghgAbateFMs.rename(columns={
        "techFMs": "Technology",
        "r": "Region",
        "ghgAbateTechFMs": "ghgAbateFMs"
    })
    training_df = X_all.merge(df_costTechFMs_renamed, on=["Region", "Technology", "year"], how="left")
...
def train_and_predict_ghgAbateTechAgri_ensemble(
    df_ghgAbateTechAgri: pd.DataFrame,
    final_feature_array_all,
    n_folds: int = 5
):
    # Step 0: Ensure final_feature_array_all is a DataFrame
    ...
    # Step 1: Cross-join final_feature_array_all with years
    years_df = pd.DataFrame(df_ghgAbateTechAgri["year"].unique(), columns=["year"])
    temp = final_feature_array_all.copy()
    temp["key"] = 1
    years_df["key"] = 1
    X_all = temp.merge(years_df, on="key").drop("key", axis=1)
    ...
    # Step 2: Merge with costTechFMs target
    df_costTechFMs_renamed = df_ghgAbateTechAgri.rename(columns={
        "techAgri": "Technology",
        "r": "Region",
        "ghgAbateTechAgri": "ghgAbateTechAgri"
    })
    training_df = X_all.merge(df_costTechFMs_renamed, on=["Region", "Technology", "year"], how="left")
    training_df = training_df.dropna(subset=["ghgAbateTechAgri"])
    ...
    # Step 4: Assemble input (X) and target (y)
    X = pd.concat([encoded_df, training_df.drop(columns=categorical_cols + ["ghgAbateTechAgri"])], axis=1)
    y = training_df["ghgAbateTechAgri"]
...
def train_and_predict_capAgri_ensemble(
    df_capAgri: pd.DataFrame,
    final_feature_array,
    n_folds: int = 5
):
    # Step 0: Ensure final_feature_array is a DataFrame
    ...
    training_df = X_all.merge(df_capAgri_renamed, on=["Region", "Technology", "year"], how="left")
    ...
    # Step 4: Assemble input (X) and target (y)
    X = pd.concat([encoded_df, training_df.drop(columns=categorical_cols + ["capAgri"])], axis=1)
...
def train_and_predict_costTechAgri_ensemble(
    df_costTechAgri: pd.DataFrame,
    final_feature_array,
    n_folds: int = 5
):
    # Step 0: Ensure final_feature_array is a DataFrame
    ...
    # Step 2: Merge with capFMs target
    df_capAgri_renamed = df_costTechAgri.rename(columns={
        "techAgri": "Technology",
        "r": "Region"
    })
    training_df = X_all.merge(df_capAgri_renamed, on=["Region", "Technology", "year"], how="left")
    ...
    # Step 4: Assemble input (X) and target (y)
    X = pd.concat([encoded_df, training_df.drop(columns=categorical_cols + ["costTechAgri"])], axis=1)
