...
def compute_agri_trend(df, value_col, feature_prefix):
    ...
    # Rename only if keys present and not already normalized
    if set(df.columns) >= {'*', '*.1', '*.2', 'Value'}:
        df = df.rename(columns=col_map)
    # Defensive: make sure Year is int (if present)
    if 'Year' in df.columns:
        df['Year'] = pd.to_numeric(df['Year'], errors='coerce')
    # Defensive: check all required columns
    required = {'Year', 'Region', 'Technology', value_col}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing columns for trend computation: {missing} in columns {df.columns}")
    # --- Step 2: Compute aggregated values ---
    df_agg = df.groupby(["Year", "Region", "Technology"])[value_col].sum().reset_index()
    print(df_agg.head())
    extracted_features = []
    # --- Step 3: For each Region-Technology pair, compute slope etc. ---
    for (region, technology), group in df_agg.groupby(["Region", "Technology"]):
        year = group["Year"]
        values = group[value_col]
        if len(year) < 2:
    ...
    # --- Step 4: Return features as DataFrame ---
