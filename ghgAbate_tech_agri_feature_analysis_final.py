...
for suffix in scenario_suffixes:
    ...
    if not os.path.exists(os.path.join(output_dir, "ghgAbateAgri_results.csv")):
        extract_gdx_results(gdx_file, output_root, sub_dir=sub_dir)
    
    # Load capAgri CSV if available
    ghgAbateTechAgri_path = os.path.join(output_dir, "ghgAbateAgri_results.csv")
    
    print(ghgAbateTechAgri_path)
    
    if os.path.exists(ghgAbateTechAgri_path):
        df_cap = pd.read_csv(ghgAbateTechAgri_path)
        ghgAbateTechAgri_dict[suffix] = df_cap
    else:
        print(f"⚠️ ghgAbateAgri_results.csv not found for: {suffix}")

print(f"✅ Loaded capAgri for {len(ghgAbateTechAgri_dict)} scenarios.")
...
for suffix in scenario_suffixes:
    ...
    # Load capAgri CSV if available
    capAgri_path = os.path.join(output_dir, "capAgri_results.csv")
    
    print(capAgri_path)
    
    if os.path.exists(capAgri_path):
        df_cap = pd.read_csv(capAgri_path)
...
# Initialize lists
ghgAbateTechAgri_dict_all = []
features_all = []

# Get common keys between both dicts
common_keys = set(ghgAbateTechAgri_dict) & set(fully_enhanced_arrays)

for key in sorted(common_keys):
    abate_df = ghgAbateTechAgri_dict[key].copy()
    feat_df = fully_enhanced_arrays[key].copy()

    # Sanity checks
    if not isinstance(abate_df, pd.DataFrame):
        raise TypeError(f"ghgAbateTechAgri_dict[{key}] is not a DataFrame.")
    if not isinstance(feat_df, pd.DataFrame):
        raise TypeError(f"fully_enhanced_arrays[{key}] is not a DataFrame.")

    ghgAbateTechAgri_dict_all.append(abate_df)
    features_all.append(feat_df)

# Concatenate vertically
ghgAbateTechAgri_dict_all = pd.concat(ghgAbateTechAgri_dict_all, axis=0).reset_index(drop=True)
...
from RenewableEnergyLanguageModel.random_forest_module import (train_and_predict_ghgAbateTechAgri_ensemble)
...
# Rename for clarity
df_targets_parts = [ghgAbateTechAgri_dict_all.iloc[i * rows_per_target:(i + 1) * rows_per_target] 
...
for i in range(len(df_targets_parts[:num_feature_parts-0])):
    result = train_and_predict_ghgAbateTechAgri_ensemble(
        df_targets_parts[i],
        df_features_parts[i],
        n_folds=17)
...
print(f"🌍 Global Voting RMSE: {rmse_global:.2f}")

print(
    f"📏 Original scale metrics:\n📈 R² Score: {r2_global:.4f}\n📉 RMSE: {rmse_global:.2f}"
)
...
print(f"🌍 Global Voting R²: {r2_global:.4f}")
print(f"🌍 Global Voting RMSE: {rmse_global:.2f} €/ha")
...
from RenewableEnergyLanguageModel.correlations_module import (plot_scenario_correlations,
                                                   ...
                                                   plot_ghgAbateTechAgri_correlation,
...
ghgAbateTechAgri_dict_aliased = replace_dict_keys(ghgAbateTechAgri_dict, alias_map)
...
correlation_matrix_ghgAbateTechAgri = plot_ghgAbateTechAgri_correlation(ghgAbateTechAgri_dict_aliased)
linkage_matrix_output = plot_scenario_dendrogram(correlation_matrix_ghgAbateTechAgri)
most_similar_ghgAbateTechAgri, max_corr_cost, least_similar_cost, min_corr_cost = find_most_and_least_similar_scenarios(correlation_matrix_ghgAbateTechAgri)
...
plt.plot(ghgAbateTechAgri_dict['CO2price_0.8_FMsgrowth_0.8_BeechArea0_0.8']["ghgAbateTechAgri"])

plt.plot(ghgAbateTechAgri_dict['Agrigrowth_0.4_Agriarea0_0.4_PeatExtract_0.4']["ghgAbateTechAgri"])


plt.plot(ghgAbateTechAgri_dict["costMargAgri_2.0_costInvAgri_2.0_costInvLevelAgri_2.0"]["ghgAbateTechAgri"])

plt.plot(ghgAbateTechAgri_dict['Agrigrowth_0.4_Agriarea0_0.4_PeatExtract_0.4']["ghgAbateTechAgri"])


"costMargAgri_2.0_costInvAgri_2.0_costInvLevelAgri_2.0"
...
result = interpret_stakeholder_query_with_prompt(
    ...
    correlation_matrix_ghgAbateTechAgri  # Or input correlation matrix
)
