...
for suffix in scenario_suffixes:
    ...
    if not os.path.exists(os.path.join(output_dir, "ghgAbateFMs_results.csv")):
        extract_gdx_results(gdx_file, output_root, sub_dir=sub_dir)
    
    # Load capFMs CSV if available
    cap_path = os.path.join(output_dir, "ghgAbateFMs_results.csv")
    
    print(cap_path)
    
    if os.path.exists(cap_path):
        df_cap = pd.read_csv(cap_path)
        ghgAbateFMs_dict[suffix] = df_cap
    else:
        print(f"⚠️ ghgAbateFMs_results.csv not found for: {suffix}")

print(f"✅ Loaded ghgAbateFMs for {len(ghgAbateFMs_dict)} scenarios.")
...
# Initialize lists
ghgAbateFMs_dict_all = []
features_all = []

# Get common keys between both dicts
common_keys = set(ghgAbateFMs_dict) & set(fully_enhanced_arrays)

for key in sorted(common_keys):
    cap_df = ghgAbateFMs_dict[key].copy()
    feat_df = fully_enhanced_arrays[key].copy()

    # Sanity checks
    if not isinstance(cap_df, pd.DataFrame):
        raise TypeError(f"ghgAbateFMs_dict[{key}] is not a DataFrame.")
    if not isinstance(feat_df, pd.DataFrame):
        raise TypeError(f"fully_enhanced_arrays[{key}] is not a DataFrame.")

    ghgAbateFMs_dict_all.append(cap_df)
    features_all.append(feat_df)

# Concatenate vertically
ghgAbateFMs_dict_all = pd.concat(ghgAbateFMs_dict_all, axis=0).reset_index(drop=True)
...
ghgAbateFMs_dict_aliased = replace_dict_keys(ghgAbateFMs_dict, alias_map)
...
plot_cost_output_correlation_heatmap(correlation_matrix_ghgAbateFMs)
...
result = interpret_stakeholder_query_with_prompt(
    ...
    correlation_matrix_ghgAbateFMs  # Or input correlation matrix
)
...
from RenewableEnergyLanguageModel.random_forest_module import (
                                                    train_and_predict_ghgAbateFMs_ensemble,
...
rows_per_target = 3472 * 30
num_target_parts = 1

# Number of rows per part
rows_per_feature = 112 * 30
num_feature_parts = 1

# Rename for clarity
df_targets_parts = [ghgAbateFMs_dict_all.iloc[i * rows_per_target:(i + 1) * rows_per_target] 
...
for i in range(len(df_targets_parts[:num_feature_parts-0])):
    result = train_and_predict_ghgAbateFMs_ensemble(
        df_targets_parts[i],
        df_features_parts[i],
        n_folds=30)
