...
for suffix in scenario_suffixes:
    ...
    if not os.path.exists(os.path.join(output_dir, "costTechAgri.csv")):
        extract_gdx_results(gdx_file, output_root, sub_dir=sub_dir)
    
    # Load capAgri CSV if available
    costTechAgri_path = os.path.join(output_dir, "costTechAgri.csv")
    
    print(costTechAgri_path)
    
    if os.path.exists(costTechAgri_path):
        df_cap = pd.read_csv(costTechAgri_path)
        costTechAgri_dict[suffix] = df_cap
    else:
        print(f"⚠️ costTechAgri.csv not found for: {suffix}")

print(f"✅ Loaded capAgri for {len(costTechAgri_dict)} scenarios.")
...
# Initialize lists
costTechAgri_dict_all = []
features_all = []

# Get common keys between both dicts
common_keys = set(costTechAgri_dict) & set(fully_enhanced_arrays)

for key in sorted(common_keys):
    cap_df = costTechAgri_dict[key].copy()
    feat_df = fully_enhanced_arrays[key].copy()

    # Sanity checks
    if not isinstance(cap_df, pd.DataFrame):
        raise TypeError(f"costTechAgri_dict[{key}] is not a DataFrame.")
    if not isinstance(feat_df, pd.DataFrame):
        raise TypeError(f"fully_enhanced_arrays[{key}] is not a DataFrame.")

    costTechAgri_dict_all.append(cap_df)
    features_all.append(feat_df)

# Concatenate vertically
costTechAgri_dict_all = pd.concat(costTechAgri_dict_all, axis=0).reset_index(drop=True)
...
from RenewableEnergyLanguageModel.random_forest_module import (train_and_predict_costTechAgri_ensemble)
...
# Rename for clarity
df_targets_parts = [costTechAgri_dict_all.iloc[i * rows_per_target:(i + 1) * rows_per_target] 
...
for i in range(len(df_targets_parts[:num_feature_parts-0])):
    result = train_and_predict_costTechAgri_ensemble(
        df_targets_parts[i],
        df_features_parts[i],
        n_folds=17)
...
costTechAgri_dict_aliased = replace_dict_keys(costTechAgri_dict, alias_map)
...
plt.plot(costTechAgri_dict['CO2price_0.8_FMsgrowth_0.8_BeechArea0_0.8']["costTechAgri"])

plt.plot(capAgri_dict['Agrigrowth_0.4_Agriarea0_0.4_PeatExtract_0.4']["capAgri"])


plt.plot(costTechAgri_dict["costMargAgri_2.0_costInvAgri_2.0_costInvLevelAgri_2.0"]["costTechAgri"])

plt.plot(costTechAgri_dict['Agrigrowth_0.4_Agriarea0_0.4_PeatExtract_0.4']["costTechAgri"])
...
result = interpret_stakeholder_query_with_prompt(
    ...
    correlation_matrix_costTechAgri  # Or input correlation matrix
)
